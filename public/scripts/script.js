...
getData().then(()=>{
    for(let i=0; i<hospital_data.length; i++){
        $('#hospital-list').append(
            ...
            '> View More </a>' + '<form action="/hosp?id=' + hospital_data[i].id +'&_method=DELETE"' + ' id="' + hospital_data[i].id + '" method="POST">' +
            '<button class="hosp btn btn-danger" >Delete Hospital</button>' +
            '</form>'         
        );
    }
}).catch((err)=>{
